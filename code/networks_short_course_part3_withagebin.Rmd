...
author: "TingFang Lee, Ashley Buchanan, URI NCIPHER Team"
date: "2026-08-03"
output:
...
}   # sna package is used in part 2, but it will mess up 
    # with igraph function in part 3, so we need to unload it here.


# Source required R scripts directly from GitHub
source("https://raw.githubusercontent.com/uri-ncipher/ASA-JSM-Causal-Inference-in-Networks-Short-Course-2026/refs/heads/main/code/IPW_functions.R")
source("https://raw.githubusercontent.com/uri-ncipher/ASA-JSM-Causal-Inference-in-Networks-Short-Course-2026/refs/heads/main/code/gcomp_dr_v2.R")
...
nodes <- read.csv(
  "https://raw.githubusercontent.com/uri-ncipher/Causal-Inference-and-Networks-Short-Course/refs/heads/main/Data/TRIPsim_nodes.csv")
edges <- read.csv(
  "https://raw.githubusercontent.com/uri-ncipher/Causal-Inference-and-Networks-Short-Course/refs/heads/main/Data/TRIPsim_edges.csv")

head(nodes)
head(edges)

# create network based on the nodes and edges data
...
plot(net0, vertex.size=1, vertex.label=NA, vertex.color="cadetblue3", 
...
# the second column is number of nearest neighbors for each participant (is also 
# node degree), the third column indicate which component the participant is in.
...
# Note: show code for averaging "age10" and "agebin" as examples. Average of 
#       other neighbor covariates already in dataset
#       Example code: df$avg_var1=unlist(lapply(1:n, avg_neighbors, net=net0, 
#                           variable="var1")) # "var1" can be any covariate name
...
avg_covariate=c("avg_age10","avg_posneg", "avg_date", "avg_share", "avg_education",
                "avg_employment")
```
...
avg_covariate=c("avg_agebin","avg_posneg", "avg_date", "avg_share", "avg_education",
                "avg_employment")
...
### IPW1 Model Fit Checks

```{r, fig.width=8, fig.height=10}

#Goodness of Fit test for random effect in mixed effect model for IPW1.
...
formula_glmer <- as.formula(paste("treatment ~", paste(
  base_covariate, collapse = "+"), "+ (1 | group)"))
formula_glmer
fit_glmer <- glmer(formula_glmer, data=nn_data, family=binomial(link="logit"))


### Fit a survival::clogit() model
formula_clogit <- as.formula(paste("treatment ~", paste(
  base_covariate, collapse = "+"), "+ strata(group)"))
formula_clogit
fit_clogit <- survival::clogit(formula_clogit, data=nn_data, method="exact")

variable_names <- list(DV="treatment", grouping="group")

TC_test <- testGOF(data=nn_data, 
...
### Check covariate balance for generalized propensity score: Fit a model on each 
### covariate, then include the individual exposure, group level exposure, and 
### propensity scores, compare to a model with only the propensity scores


# IPW2 was fitted using "age10", so we check balance using "age10" (continuous 
# covariate), not "agebin" (binary covariate)
base_covariate=c("age10", "posneg", "date", "share", "education", "employment")
avg_covariate=c("avg_age10","avg_posneg", "avg_date", "avg_share", "avg_education",
                "avg_employment")
...
for (i in 1:length(base_covariate)) {
  ...
  ipw2_balance <- rbind(ipw2_balance, c(f1, f2, round(res_temp$Chisq[2], 4), 
                                        round(res_temp$`Pr(>Chisq)`[2], 4))
  )
}
...
make_latex_effect_table <- function(results,
  ...
  if (direct_n > 0) {
    tab <- tab %>% pack_rows("Direct effects", start, start + direct_n - 1, 
                             italic = TRUE)
    start <- start + direct_n
  }

  if (indirect_n > 0) {
    tab <- tab %>% pack_rows("Indirect effects", start, start + indirect_n - 1, 
                             italic = TRUE)
    start <- start + indirect_n
  }

  if (total_n > 0) {
    tab <- tab %>% pack_rows("Total effects", start, start + total_n - 1, 
                             italic = TRUE)
    start <- start + total_n
  }

  if (overall_n > 0) {
    tab <- tab %>% pack_rows("Overall effects", start, start + overall_n - 1, 
                             italic = TRUE)
  }
