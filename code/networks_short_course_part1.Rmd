...
var_labels <- c(
  posneg     = "HIV status",
  share      = "Shared needles",
  education  = "Education",
  employment = "Employment status",
  treatment  = "Treatment assignment",
  outcome    = "Outcome",
  num_nn     = "Number of network neighbors",
  num_tnn    = "Number of treated neighbors",
  num_utnn   = "Number of untreated neighbors"
)
...
make_table1 <- function(data, cat_vars, num_vars,
  ...
  cat_table <- purrr::map_dfr(cat_vars, function(var) {

    # Header row
    header <- tibble(
      Level = if (!is.null(var_labels)) var_labels[[var]] else var,
      `Count (%)` = ""
    )
    ...
    if (!is.null(level_labels) && var %in% names(level_labels)) {
      rows$Level <- dplyr::recode(rows$Level, !!!level_labels[[var]])
    }
    ...
    rows <- rows %>%
      transmute(
        Level = paste0("  ", Level),
        `Count (%)` = paste0(
          n, " (", scales::percent(n / sum(n), accuracy = 0.1), ")"
        )
      )
  ...
  # Numeric variables
  num_table <- purrr::map_dfr(num_vars, function(var) {
...
### Check the positivity assumption for random violations, create tables of the exposure to the community alerts by each covariate. Based on the exposure, there are no structural violations of positivity (exposure possible for all combinations of individual and group level covariates).
...
### Estimation using Inverse Probability Weighted Estimator (assumes exposure propensity score model correctly specified)
...
# Under SUTVA and IID, we sample each individual with replacement for the bootstrap samples
...
#Obtain 2.5% and 97.5% of bootstrap distribution for parameter of interest
...
### Estimation using an Augmented IPW Estimator (doubly robust). Two estimators coded below are augmented IPW (DR1, Lunceford and Davidian 2004) and weighted regression estimator (DR2, Robins 2007).
...
withCallingHandlers(
  ...
  #This warning is expected because we have binary outcomes
  warning = function(w) {
...
### Assess the fit of the propensity score model using overlap plots and standardized mean differences before and after weighting.

```{r}
############################################################
# 9. Assess the propensity score model and outcome model fit
############################################################
...
overlap <-ggplot(nodes, aes(x = denom.p, color = factor(treatment), fill = factor(treatment))) +
...
ggsave(
  filename = "propensity_overlap.pdf",
  plot = overlap,
  width = 8,
  height = 7
)
...
love <- love.plot(
  ...
  title = "Absolute standardized mean differences before and after IPW"
)

ggsave(
  filename = "loveplot.pdf",
  plot = love,
  width = 8,
  height = 7
)

# Check fit of GEE model

glm.obj <- glm( outcome ~ treatment + age10 + posneg 
                + date + share + education + employment, data = nodes, 
                family = binomial(link = "logit") ) 

nodes$pred <- predict(glm.obj, type = "response") 

nodes$pred_group <- cut( nodes$pred, breaks = quantile(nodes$pred, seq(0, 1, 0.1), na.rm = TRUE), include.lowest = TRUE ) 

cal <- aggregate( cbind(observed = nodes$outcome, predicted = nodes$pred), by = list(pred_group = nodes$pred_group), FUN = mean )
